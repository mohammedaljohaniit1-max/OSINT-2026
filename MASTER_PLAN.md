# 🕵️ OSINT-2026 — خارطة Argus الهندسية

> **الاسم الكودي:** `Argus` — العملاق ذو المئة عين.
> **الحالة:** خارطة طريق؛ البنود لا تُعد منفذة إلا إذا كانت مذكورة في README ومغطاة باختبار.
> **آخر تحديث:** 2026-09-02
> **عقد الحقيقة:** لا توجد أداة OSINT معتمدة 100% ولا يمكن ضمان التفوق المطلق على كل الأدوات. هدف Argus هو التفوق القابل للقياس في الدقة، ومنع الإيجابيات الكاذبة، ووضوح فجوات التغطية.

## بوابات إصدار «الأقوى بالدليل»

| البوابة | المعيار الإلزامي |
|---|---|
| دقة ملكية الحساب | Precision منشور على مجموعة بيانات معلّمة، ولا تُقبل نتيجة HTTP 200 كإثبات |
| الإيجابيات الكاذبة | Negative controls وصفحات soft-404/generic/login-wall ضمن اختبارات الانحدار |
| دمج الأشخاص | قياس false-merge وfalse-split؛ الاسم والمدينة وحدهما لا يدمجان هويتين |
| قابلية التكرار | إصدار Argus، إصدارات الأدوات، إعدادات الفحص، وحالة كل مصدر داخل manifest |
| التغطية | success/empty/partial/failed/timeout/unavailable/skipped لكل وحدة |
| المقارنة | نفس targets والميزانية الزمنية مقابل Sherlock وMaigret وأطر orchestration المختارة |
| الأمان | TLS افتراضي، active opt-in، حدود استجابة وزمن، واحترام النطاق القانوني |

## الحالة الواقعية للإصدار 2.1

**منفذ:** truth states، evidence provenance/families، confidence محدود، execution ledger، TLS آمن، negative controls لمنصات مختارة، persona fusion محافظ، وتقارير coverage/limitations.

**قيد التنفيذ:** benchmark CLI، corpus مرجعي، structured adapters، source health history، وmanifest أعمق للمواد الخام.

**مخطط وليس منفذاً بعد:** PDF/CSV/STIX، browser automation، dashboard، وكل تكامل مذكور لاحقاً ولا يظهر في `argus modules`. لا يجوز استخدام هذه الوثيقة كدليل أن الميزة موجودة.
